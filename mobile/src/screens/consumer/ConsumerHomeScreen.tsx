...
import { QrCode, Coins, Star, Settings } from 'lucide-react-native';
    ...
    <SafeAreaView className="flex-1 bg-navy-900 px-4">
      ...
      <View className="bg-gradient-to-r from-consumer/20 to-navy-800 border border-consumer/30 rounded-3xl p-6 mb-8 relative overflow-hidden">
        ...
        <View className="flex-row items-baseline gap-2 mb-4">
          <Text className="text-consumer text-5xl font-black">{user?.tokens || 1450}</Text>
      ...
      <View className="flex-1">
        <Text className="text-white text-lg font-bold mb-4">Recent Scans</Text>
        {[1, 2].map((item) => (
          <View key={item} className="flex-row items-center bg-navy-800 rounded-xl p-4 mb-3 border border-white/5">
            ...
            <View className="flex-1">
              <Text className="text-white font-bold">Organic Tomatoes Batch #{8922 - item}</Text>
              <Text className="text-gray-400 text-xs mt-1">Trust Score: 94 • 2 days ago</Text>
            </View>
            <View className="bg-green-accent/20 px-2 py-1 rounded">
              <Text className="text-green-accent text-xs font-bold">+5</Text>
            </View>
          </View>
        ))}
      </View>
