...
const navItems = [
  ...
  { icon: AlertTriangle, label: 'Alerts', path: '/dashboard/alerts', badge: 3 },
            ...
            return (
              ...
              >
                ...
                {item.badge && !collapsed && (
                  <span className="ml-auto text-[10px] font-bold bg-accent-red/20 text-accent-red px-1.5 py-0.5 rounded-full">
                    {item.badge}
                  </span>
                )}
                {item.badge && collapsed && (
        ...
        {!collapsed && (
          <div className="p-3 border-t border-white/[0.06]">
            <div className="flex items-center gap-3 p-2 rounded-xl bg-navy-600/30">
              <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-accent-green to-accent-blue flex-shrink-0" />
              <div className="min-w-0">
                <p className="text-xs font-semibold truncate">Admin User</p>
                <p className="text-[10px] text-gray-500 truncate">admin@origyn.com</p>
              </div>
              <LogOut size={14} className="text-gray-500 hover:text-white cursor-pointer ml-auto flex-shrink-0" />
            </div>
          </div>
        )}
      </aside>
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="h-14 border-b border-white/[0.06] bg-navy-800/30 backdrop-blur-xl flex items-center justify-between px-4 lg:px-6 flex-shrink-0 z-10">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-accent-green to-accent-blue border-2 border-navy-800 cursor-pointer" />
          </div>
        </header>
